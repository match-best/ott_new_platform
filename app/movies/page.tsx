...
import { Search, Filter, X, Sparkles, Film, Calendar, Star, ChevronDown } from "lucide-react"
...
export default function MoviesPage() {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (movie) =>
          ...
          movie.cast?.some((actor: string) => actor.toLowerCase().includes(searchTerm.toLowerCase()))
      )
    }
    ...
    if (selectedGenres.length > 0) {
      filtered = filtered.filter((movie) => 
        movie.genre && Array.isArray(movie.genre) && selectedGenres.some((genre) => movie.genre.includes(genre))
      )
    }
  ...
  }, [movies, searchTerm, selectedGenres, selectedYear, selectedRating, sortBy])

  const allGenres = Array.from(new Set(movies.flatMap((movie) => movie.genre || [])))
  const allYears = Array.from(new Set(movies.map((movie) => movie.year).filter(Boolean))).sort((a, b) => b - a)
  ...
  const activeFiltersCount = selectedGenres.length + (selectedYear ? 1 : 0) + (selectedRating ? 1 : 0)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-black flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-white text-3xl font-bold mb-3 tracking-wide">Loading Movies</div>
          <div className="text-purple-300 text-lg">Preparing your cinema experience...</div>
        </div>
      </div>
    )
  }
        ...
        <div className="container mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-400 text-lg">
              {filteredMovies.length} movies available to stream
            </p>
          </div>
